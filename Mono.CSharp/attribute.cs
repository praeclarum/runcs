...
namespace Mono.CSharp {
	...
	public class Attribute : Expression
	{
		...
		public bool CheckTarget ()
		{
			...
			if (System.Linq.Enumerable.Any(valid_targets, i => i == ExplicitTarget)) {
...
//        }
		...
		{
			return (CharSet)System.Enum.Parse (typeof (CharSet), ((Constant) PosArguments [0].Expr).GetValue ().ToString (), true);
		}
		...
		public bool IsInternalCall ()
		{
			...
			if (PosArguments.Count == 1) {
				options = (MethodImplOptions) System.Enum.Parse (typeof (MethodImplOptions), ((Constant) PosArguments[0].Expr).GetValue ().ToString (), true);
			} else if (HasField ("Value")) {
				var named = GetNamedValue ("Value");
				options = (MethodImplOptions) System.Enum.Parse (typeof (MethodImplOptions), named.GetValue ().ToString (), true);
			}
		...
		public bool IsExplicitLayoutKind ()
		{
			...
			var value = (LayoutKind) System.Enum.Parse (typeof (LayoutKind), ((Constant) PosArguments[0].Expr).GetValue ().ToString (), true);
					...
					for (int j = 0; j < PosArguments.Count; ++j) {
						...
						if (j == 0) {
							if (Type == predefined.IndexerName || Type == predefined.Conditional) {
								...
								if (PosArguments.Count == 1) {
									var u_type = (UnmanagedType) System.Enum.Parse (typeof (UnmanagedType), ((Constant) PosArguments[0].Expr).GetValue ().ToString (), true);
	...
	public class PredefinedAttribute : PredefinedType
	{
		...
		public PropertySpec GetProperty (string name, TypeSpec memberType, Location loc)
		{
			...
			if (properties != null) {
				spec = System.Linq.Enumerable.FirstOrDefault (properties, l => l.Name == name);
			} else {
				spec = null;
			}
